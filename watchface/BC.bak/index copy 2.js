...
// Ring holds a small icon + the value, stacked and centered inside it.
// The label now lives OUTSIDE the ring, below its bottom edge, so it never
// visually competes with the number - matching the reference layout.
const TOP_CY = 108
const TOP_R = 38
const TOP_GLOW_R = 42
const TOP_LINE_W = 7

const TOP_ICON_CY = TOP_CY - 15 // icon glyph, upper half of the ring
const TOP_VALUE_CY = TOP_CY + 11 // value text, lower half of the ring
const TOP_LABEL_CY = TOP_CY + TOP_R + 20 // caption, below the ring entirely

const HR_CX = 128
const STEPS_CX = 240
const BAT_CX = 352

// Thin vertical separators between the three top indicators
const TOP_DIVIDER_TOP = TOP_CY - TOP_R - 6
const TOP_DIVIDER_BOTTOM = TOP_CY + TOP_R + 6
const TOP_DIVIDER_X1 = (HR_CX + STEPS_CX) / 2
const TOP_DIVIDER_X2 = (STEPS_CX + BAT_CX) / 2
...
const TIME_CY = 230
const TIME_FONT_SIZE = 80 // widened from 72 per request
...
const TIME_GLOW_COLOR = 0x123a44
const TIME_GLOW_OFFSETS = [
  [-2, 0], [2, 0], [0, -2], [0, 2],
  [-1, -1], [1, -1], [-1, 1], [1, 1]
]

const TIME_HHMM_BOX_W = 220 // reserved width for "HH:MM" at TIME_FONT_SIZE
const TIME_COLON_W = 32
const TIME_SEC_DIGIT_W = 58
...
const DATE_LINE_CY = 302
const DATE_LINE_FONT_SIZE = 20
...
// ============================================================
// SMALL VECTOR ICONS (drawn with primitives - no image assets needed)
// ============================================================
...
// ============================================================
// Each indicator: ring -> [icon][value] stacked inside, caption below the
// ring outside it, thin vertical dividers between the three columns.
...
function createTopWidgets() {
  ...
  createWidget(widget.LINE, {
    x1: TOP_DIVIDER_X1, y1: TOP_DIVIDER_TOP,
    x2: TOP_DIVIDER_X1, y2: TOP_DIVIDER_BOTTOM,
    color: COLOR_TRACK,
    line_width: 2
  })
  createWidget(widget.LINE, {
    x1: TOP_DIVIDER_X2, y1: TOP_DIVIDER_TOP,
    x2: TOP_DIVIDER_X2, y2: TOP_DIVIDER_BOTTOM,
    color: COLOR_TRACK,
    line_width: 2
  })
  ...
  drawPulseIcon(HR_CX, TOP_ICON_CY, COLOR_WHITE)
  ...
  drawStepsIcon(STEPS_CX, TOP_ICON_CY, COLOR_WHITE)
  ...
  drawBatteryIcon(BAT_CX, TOP_ICON_CY, COLOR_WHITE)
...
function createMainTime() {
  ...
  for (let i = 0; i < TIME_GLOW_OFFSETS.length; i++) {
    ...
    const glow = createWidget(widget.TEXT, {
      x: TIME_HHMM_X + dx,
      y: boxY + dy,
      w: TIME_HHMM_BOX_W,
      h: TIME_ROW_H,
...
function createDate() {
  dateLineWidget = createWidget(widget.TEXT, {
    x: 0,
    y: DATE_LINE_CY - 14,
    w: SCREEN_W,
    h: 28,
